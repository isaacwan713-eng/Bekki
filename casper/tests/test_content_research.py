...
class ContentResearchTests(unittest.TestCase):
    def test_content_discovery_merges_two_browser_engines_for_ai_ranking(self):
        google_item = {
        ...
        def fake_search(_query, count=7, engine="duckduckgo"):
            engines.append(engine)
            item = google_item if engine == "google" else duckduckgo_item
        ...
        with patch.object(
            ...
            return_value=("google", "duckduckgo"),
        ):
        ...
        self.assertEqual(engines, ["google", "duckduckgo"])
        self.assertEqual(
            result["discovery_type"],
            "casper_browser_multi_engine",
        )

    def test_ai_controls_engine_plan_and_receives_current_task(self):
        ...
        self.assertIn('"country_code": "US"', model.call_args.args[1])
        self.assertIn("Football Manager 2026", model.call_args.args[1])

    def test_ai_controls_china_engine_plan(self):
        model = unittest.mock.Mock(
            return_value={
                "engines": ["baidu", "sogou"],
        ...
        self.assertEqual(engines, ("baidu", "sogou"))

    def test_ai_can_select_naver_for_korea(self):
        model = unittest.mock.Mock(
            return_value={
        ...
        self.assertEqual(engines, ("naver", "google"))

    def test_invalid_ai_engine_plan_retries_then_fails_closed(self):
        model = unittest.mock.Mock(return_value={"engines": ["unknown"]})
        tools_stub = types.SimpleNamespace(run_ai_prompt=model)
        content_research.browser._ai_search_engine_policy.cache_clear()
        with patch.dict(sys.modules, {"tools": tools_stub}):
    ...
    def test_duckduckgo_redirect_is_unwrapped_without_semantic_filtering(self):
        wrapped = (
            "https://duckduckgo.com/l/?uddg="
            "https%3A%2F%2Fcommunity.test%2Ffm26-tactic"
        )
        self.assertEqual(
            content_research.browser._unwrap_duckduckgo_url(wrapped),
            "https://community.test/fm26-tactic",
        )
    ...
    def test_query_review_uses_sanitized_third_attempt(self):
        ...
        with patch.object(
            ...
            result = content_research._review_queries("找适合曼联的战术", plan)
        third_payload = model.call_args_list[3].args[1]
    ...
    def test_confirmed_learning_checkpoint_resumes_exact_skill_candidate(self):
        procedure = {"id": "candidate-id", "target_app": "FM26"}
        expected = {"action": "installed_fm_tactic"}
        with patch(
            ...
            content_workflow, "_classify_authorized_stage"
        ) as classifier:
            result = content_workflow.execute(
                "找战术并安装",
                "context",
