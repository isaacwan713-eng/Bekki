...
def discover_web(query, count=7, status_callback=None):
...
def shopping_research_controller(
    ...
    site_scope = " OR ".join("site:" + domain for domain in merchant_domains)
    scoped_queries = ["(" + site_scope + ") " + query for query in queries[:3]]
