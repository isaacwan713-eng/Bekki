...
def save_pending_action(action, session_id=None):
    os.makedirs(DATA_FOLDER, exist_ok=True)
...
def loading_pending_action(session_id=None):
    pending = load_json_file(PENDING_FILE, {})
    if not isinstance(pending, dict) or not pending:
        return {}

    # Legacy pending actions were global and could leak into another chat.
    # They are intentionally invalidated during this migration.
    if not pending.get("session_id") or not pending.get("expires_at"):
        clear_pending_action()
        return {}

    if session_id is not None and pending.get("session_id") != str(session_id):
        return {}
