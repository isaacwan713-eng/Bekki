...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Knowledge relationship support-recovery tests failed."
    }
...
for namespace, key, payload, expected_hash in rows:
    ...
    if int(semantic.get("relationship_support_migration_version") or 0) < 1:
        topic_contract_failures.append(key + ":support_migration")
...
sihixian_member_rows = [
    edge for edge in sihixian_all
...
sihixian_names = sorted({
    ...
    for edge in sihixian_member_rows
})
...
if sihixian_member_rows and any(
    edge.get("current_active") is not True
    or edge.get("historical_active") is not True
    for edge in sihixian_member_rows
):
    projection_failures.append("sihixian_mixed_support_not_recovered")
...
print("relationship_support_migration_version =", knowledge.KNOWLEDGE_RELATIONSHIP_SUPPORT_MIGRATION_VERSION)
...
'@ | python -
    if ($LASTEXITCODE -ne 0) {
        throw "Knowledge relationship support-recovery live validation failed."
    }
}
finally {
    Pop-Location
